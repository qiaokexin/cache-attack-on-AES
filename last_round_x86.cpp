...
for (size_t b=0; b < 16; ++b){// the b-th byte
 printf("b = %ld\n", b);
 loop: for (l = 0; l < 2; ++l) // recover by l-th memory lines    
 {
...
sched_yield();
        ...
        { 
          if (inv_sBox[ciphertext[b]^key_last]/16 == l){
            H0_hit++;
            H0_total++;
          }
          else{
      ...
      sched_yield();
      Diff_ratio[byte] = (double)H1_miss / (double)H1_total - (double)H0_miss /(double) H0_total;
      
      printf("%f ", Diff_ratio[byte]);
      sched_yield();
   }//end of guesses
   max[l] = get_max_index(Diff_ratio, 256);  
   printf("\n");
  }//end of memory lines
